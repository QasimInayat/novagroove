...
@section('content')
<style>
    .pricingTable .pricing-content {list-style: disc !important;padding: 50px;}.pricingTable .pricing-content li:before {content : none !important;font-size:18px !important;}.package {-webkit-box-shadow: 0 0 30px 0 rgb(0 0 0 / 7%);box-shadow: 0 0 30px 0 rgb(0 0 0 / 7%);border: 1px solid #252525;}.package h4 {background-color: #252525;padding: 25px;font-size: 22px;}.font-oswald {font-family: 'Oswald', sans-serif;}.package span {font-size: 18px;font-weight: 300;}.text-capitalize {text-transform: capitalize!important;}.list-unstyled {padding-left: 0;list-style: none;}.btn-check:checked+.btn, .btn.active, .btn.show, .btn:first-child:active, :not(.btn-check)+.btn:active {color: var(--bs-btn-active-color);background-color: var(--bs-btn-active-bg);border-color: var(--bs-btn-active-border-color);}.package p {font-size: 36px;}.main-page p {font-size: 15px;}.btn-primary {background-color: white;color: #000 !important;border-radius: 0;border: 1px solid white;font-weight: 600;padding: 16px 37px;font-size: 14px;margin-bottom: 10px;}
</style>
        ...
        @if(isset($packages) && !empty($packages))
         ...
         <div class="col-md-4 mb-md-0 mb-4">
            <div class="package text-center text-white">
               ...
               <a href="{{route('subscribe', strtolower($value->name))}}" class="btn btn-primary text-capitalize mt-3 mb-5">Pick {{isset($value->name ) && !empty($value->name) ? $value->name : '-'}} Plan</a>
            </div>
         </div>
         @endforeach
        @endif
