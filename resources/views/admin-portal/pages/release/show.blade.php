    ...
    <div class="row">
        <div class="col-lg-12">
            <h1 class="h1 mb-4 mt-2 d-inline-block mr-3"><a href="{{route('admin.release.index')}}"><i class="mdi mdi-keyboard-return"></i></a>{{$release->album_title}} <small class="user"
                    style="color: white;">(No. of Songs - {{count(json_decode($release->track_ids))}})</small></h1>

        </div>
        <div class="col-12">
            <p class="mb-2">UPC CODE  - {{$release->upc_code}}&nbsp;<a href="javascript:;" onClick="updateRelease($(this),{{ $release->id }})"><i class="fa fa-edit" style="color:white"></i></a></p>
            <p class="mb-2">LABEL - {{isset($release->label) ? $release->label : ''}}</p>
            <p class="mb-2">Version?  - {{$release->version}}</p>
            <hr class="hr-releases">
            <p class="mb-2 mt-2" style="color: white;">Previous Released</p>
            ...
            @endif
             <hr class="hr-releases">
              ...
              @if(!empty($release->p_copyright_year))
                ...
                @endif
              <p class="mb-2">
                <span class="badge bg-default" style="background:white !important;"><a href="{{route('admin.release.albumArtWork',$release->album_artwork)}}" style="color:black;text-decoration:none">Download Art work</a></span>
              </p>
    ...
    <div class="modal-dialog" role="document">
        <form action="{{ route('admin.updateRelease',$release->id) }}" method="POST">
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-primary" style="background-color: white; color: black; border-color: white;">Save changes</button>
                </div>
            </div>
        </form>
    </div>
