...
<style>
    ...
    .modal-content {
        ...
        border: var(--bs-modal-border-width) solid white;
...
<div class="row mt-2 px-lg-4">
   ...
   <div class="col-12">
    <form id="form" method="POST" enctype="multipart/form-data" action="{{route('user.release.update',$release->id)}}" >
        @csrf
        @method('PUT')
      @if(count(collect(json_decode($release->track_ids))) > 0)
         <div id="audioTracks">
      ...
      <div class="auth-form">
         <div class="card">
            ...
            <div class="card-body py-4 px-lg-4 px-2">
               <div class="mb-lg-4 mb-3 input-icon position-relative">
                  ...
                  {!! Form::select('number_of_songs', numberOfSongs(), count(collect(json_decode($release->track_ids))), ['class' => 'form-control custom-control ', 'id' => 'number_of_songs', 'required']) !!}
                </select>
                  <i class="fa-solid fa-chevron-down select-icon"></i>
               </div>
               <div class="mb-lg-4 mb-3">
                  <label for="" class="form-label">Album Title</label>
                  <input type="text" name="album_title" id="alTitle" required placeholder="Album Title" class="form-control custom-control" value="{{ $release->album_title }}">
               </div>
               <div class="mb-lg-4 mb-3">
                <label for="" class="form-label">Has this product been previously released?</label>
                <div class="form-check ">
                    <input class="form-check-input me-3" name="is_previous_released"type="radio" id="flexRadioDefault1" value="1" onchange="previousRelease($(this))" @if($release->is_previous_released == 1) checked @endif>
                ...
                <div class="form-check ">
                    <input class="form-check-input me-3" name="is_previous_released" type="radio" value="0"  id="flexRadioDefault2"  onchange="previousRelease($(this))" @if($release->is_previous_released == 0) checked @endif>
                    <label class="form-check-label" for="flexRadioDefault2">
                    <span class="custom-label">No, It's a new release</span>

                    </label>
                </div>
                <div class="mb-lg-4 mb-3" id="previousReleaseDate" @if($release->is_previous_released == 0) style="display:none" @endif>
                    <input type="text" name="previous_release_date" id="previous_release_date"  placeholder="Pick the previous date" class="form-control custom-control mt10px input" max="{{ date('Y-m-d') }}" value="{{ $release->previous_release_date }}">
                </div>
               </div>
               <div class="mb-lg-4 mb-3">
               ...
               <div class="mb-lg-4 mb-3">
                  ...
                  <input type="text" name="label" id="label" required placeholder="Label" class="form-control custom-control" value="{{ $release->label }}">
               </div>
               <div class="mb-lg-4 mb-3">
                  <label for="" class="form-label">Version?</label>
                  <input type="text" name="version" id="version" placeholder="Leave a blank if original" class="form-control custom-control" value="{{ $release->version }}">
               </div>
                <input type="hidden" id="hiddenArtistType" value="other">
                <input type="hidden" id="hiddenRole" value="">
               @include('user-portal.pages.release.edit-artist-relase')
               ...
               <div>
                  ...
                  <div class="mb-lg-4 mb-3 position-relative">
                    ...
                    <small class="upload-msg text-danger" style="width: auto !important; display: none;"></small>
                    <div class="image-album" style="display: block;">
                      <span class="cross">x</span>
                      <img id="uploadedImage" src="{{asset(config('upload_path.album_artwork').$release->album_artwork)}}" alt="Uploaded Image" accept="image/png, image/jpeg" style="display: block; height: 150px; width: 150px;" class="img-upload-album">
                   </div>

                    <input type="file" id="readUrl" name="album_artwork"  accept="image/*" class="form-control custom-control" style="display: none;">
               ...
               <div class="mb-lg-4 mb-3 position-relative input-icon">
                  <label for="" class="form-label">Primary Language</label>
                    {!! Form::select('language', primaryLanguages(), $release->language, ['class' => 'form-control custom-control', 'id' => 'exampleFormControlSelect2', 'required']) !!}
                  <i class="fa-solid fa-chevron-down select-icon"></i>
               </div>
               ...
               <div class="mb-lg-4 mb-3 position-relative input-icon">
                ...
                <input type="text" name="catalog_id" id="catalog_id" placeholder="Add your own catalog reference ID (Optional)" class="form-control custom-control" value="{{$release->catalog_id}}">
             </div>
             ...
             <div class="d-md-flex justify-content-between gap-3">
              <div class="mb-lg-4 mb-3 w-50    position-relative input-icon">
                 ...
                 <select name="p_copyright_year" id="p_copyright_year"  required class="form-control custom-control">
                  <option value="">Select Year</option>
                    @forelse (copyrightYear() as $copyright)
                          <option value="{{$copyright}}" @if($copyright == $release->p_copyright_year) selected @endif>{{$copyright}}</option>
                    @empty
                    @endforelse
                 </select>
                 <i class="fa-solid fa-chevron-down select-icon"></i>
              ...
              <div class="mb-lg-4 mb-3 w-100 input-icon position-relative">
                 ...
                 <input type="text" name="p_copyright_name" id="p_copyright_name" placeholder="Name of individual/entity" class="form-control custom-control" value="{{$release->p_copyright_name}}" required>
              </div>
            </div>
            ...
            <div class="d-md-flex justify-content-between gap-3">
              <div class="mb-lg-4 mb-3 w-50    position-relative input-icon">
                 ...
                 <select name="c_copyright_year" id="c_copyright_year" required class="form-control custom-control">
                  <option value="">Select Year</option>
                    @forelse (copyrightYear() as $copyright)
                        <option value="{{$copyright}}" @if($copyright == $release->c_copyright_year) selected @endif>{{$copyright}}</option>
                    @empty
              ...
              <div class="mb-lg-4 mb-3 w-100 input-icon position-relative">
                 <label for="" class="form-label">&nbsp;</label>
                 <input type="text" name="c_copyright_name" id="c_copyright_name" placeholder="Name of individual/entity" class="form-control custom-control" value="{{$release->c_copyright_name}}" required>
              </div>
            </div>
                    ...
                    <div class="stories mt-2 border p-3 pb-0">
                        <div id="showTimes">
                            @if(json_decode($release->stores))
                                @forelse (storesIn(json_decode($release->stores)) as $index=>$store)
                                    @php $validate =  validateStoreStatus($store->name) @endphp
                                    @if($validate != false)
                                        <input type="hidden" name="stores[]" value="{{ $store->name }}" class="storeDetails_{{ $store->id }}">
                                        <span class="mx-2 mb-3 d-inline-block @if($store->name != 'Spotify') showTimes  @endif storeDetails_{{ $store->id }} storeIcon_{{ $store->id }}"  data-id="{{ $store->id }}"><a href="javascript:;" @if($store->name != 'Spotify') onClick="removeStore({{$store->id}})" @endif id="store_{{ $store->id }}" style="color:white"><img src="{{asset('assets/icons/'.$store->logo)}}" width="45" height="45" alt="{{ $store->name }}"></a></span>
                                    @endif
                                @empty
                                @endforelse
                            @endif
         ...
         <div class="card mt-3">
            <div class="py-4 px-lg-4 px-2" id="uploadedTracks">
                <label for="" class="form-label">Uploaded Tracks</label>
                @php $tracks =  getTracks($release->track_ids); $trackCounts = count($tracks); @endphp
                @forelse ($tracks as $index=>$item)
                    <div class="stories mt-2 border p-3" id="remove_track_{{ $item->id }}">
                        <div class="row">
                            <div class="col-1">
                                <small>&nbsp;</small>
                                <h4 >{{ ++$index }}</h4>
                            ...
                            <div class="col-9">
                                ...
                                <h4 id="track_name_{{ $item->id }}" style="color: var(--primary)">{{ $item->title }}</h4>
                            </div>
                            <div class="col-md-2">
                                <div class="row">
                                    <div class="col-12 text-center">
                                        <small>Edit</small><br>
                                        <a href="#multiTracks" onClick="editTrackv2('{{ $item->id }}','{{ ++$index }}')"><span><i class="fa fa-pencil-square" style="color:var(--primary); font-size:30px"></i></span></a>
                                    </div>
                                    <div class="col-6 d-none">
                                        <small>Remove</small><br>
                                        <a href="javascript:;" onClick="removeTrack({{ $item->id }},$(this),{{ $release->id }})"><span><i class="fa fa-minus-square" style="color:var(--primary); font-size:30px;"></i></span></a>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="card-body border-top">
                <div class="mb-lg-4 mb-3 input-icon position-relative" id="aTrack" style="display:none" data-type="hide">
                    <div class="col-lg-12">
               ...
               <div class="mb-lg-4 mb-3 input-icon position-relative" id="lyricsSection" style="display:none" data-type="hide">
                    <div class="col-lg-12">
               ...
               <div class="row">
                  <div class="col-12 text-end">
                    <div id="createMode" >
                        <a href="#multiTracks">
                            <button type="button" id="addTrack" data-id="{{ $trackCounts + 1}}" onClick="uploadTrack($(this))" class="btn btn-primary rounded" style="display:none">Add Track</button>
                        </a>
                    </div>
                    <div id="updateMode" style="display: none">
                        <a href="#multiTracks">
                            <button type="button" id="updateTrack"  data-id="{{ $trackCounts + 1}}"   class="btn btn-primary rounded">Update Track</button>
                        </a>
                        &nbsp;&nbsp;
                        <a href="#multiTracks">
                            <button type="button" id="resetTrack"  data-id="{{ $trackCounts + 1}}"  class="btn btn-primary rounded" style="color:white; background-color:black">Cancel</button>
                        </a>
                    </div>
                  </div>
               </div>
         ...
         <div class="card mt-3">
            <div class="card-body py-4 px-lg-4 px-2">
               ...
               <span class="ins d-block mt-4 mb-lg-4 mb-3 d-none">Add your First track this button will be enable on your first upload. Upload Your Track and hit on Add Track button</span>
               <div class="row">
                  <div class="col-12 ">
                    <span class="badge bg-danger mb-3 d-none" id="msg_no_of_songs" style="">You have selected <span class="no_of_songs"></span> tracks. Please upload <span class="no_of_songs"></span> tracks to enable this button</span>
                     <button id="updateRelease" type="submit" class="btn btn-primary rounded w-100" >Update Release</button>
                  </div>
               </div>
            </div>
         </div>
...
<script>
   ...
   $(".showCheck").hover(function(){
       var id = $(this).attr('data-id');
       $("#store_"+id).addClass('fa fa-check');
   });

   function removeStore(id){
    //    var html = $('.storeIcon_'+id).html();
       $('.storeDetails_'+id).remove();
    //    $("#showCheck").append(html);
   }
</script>
<script>
   $(document).ready(function () {
   ...
   function multiTrack(count, trackId, track_title) {
    //   $("#multiTracks").append('<div class="card-body py-4 px-lg-4 px-2"><div class="row mt-5  " id="track_msg_' + count + '" > <div class="col-md-12"> <h2 class="text-center trackUploaded" ><i class="fa fa-check text-success"></i>&nbsp;&nbsp;('+count+') ' + track_title + ' Uploaded</h2> <div class="text-center"> <a href="javascript:;" onClick="editTrack(' + trackId + ',$(this))"  data-count="' + count + '" data-name="' + track_title + '"><i class="fa fa-pencil text-success d-none" style="font-size:25px"></i></a>&nbsp;&nbsp;<a href="javascript:;" onClick="deleteTrack(' + trackId + ',$(this))" data-count="' + count + '" data-name="' + track_title + '"><i class="fa fa-trash text-danger" style="font-size:25px;"></i></a> </div></div> </div></div>');
      $("#uploadedTracks").append('<div id="remove_track_'+trackId+'" class="stories mt-2 border p-3"><div class="row"><div class="col-1"><small>&nbsp;</small><h4 >'+count+'</h4></div><div class="col-9"><small>Track Name</small><h4 id="track_name_'+trackId+'"style="color: var(--primary)">'+track_title+'</h4></div><div class="col-md-2"><div class="row"><div class="col-12 text-center"><small>Edit</small><br><a href="#multiTracks" onClick="editTrackv2('+trackId+','+count+')"><span><i class="fa fa-pencil-square" style="color:var(--primary); font-size:30px"></i></span></a></div><div class="col-6 d-none"><small>Remove</small><br><a href="javascript:;" onClick="removeTrack('+trackId+',$(this),{{ $release->id }})"><span><i class="fa fa-minus-square" style="color:var(--primary); font-size:30px;"></i></span></a></div></div></div></div></div>');
   }
   ...
   function previousRelease(elm){
        ...
        if(value == 1){
            $("#previousReleaseDate").show().attr('required',true);
        }
        else{
            $("#previousReleaseDate").hide().attr('required',false);
