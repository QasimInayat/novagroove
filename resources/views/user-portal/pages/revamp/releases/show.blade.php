...
<div class="row mt-2 px-lg-4">
    <div class="col-12">
        <div class="mt-1">
            <div>
                <h2 class="user-name">
                    <span class="me-2"><a href="{{route('user.release.index')}}"><i class="fa-solid fa-arrow-left" style="color:white"></i></a></span> {{$release->album_title}} <span>(No. of Songs - {{count(json_decode($release->track_ids))}})</span>
                </h2>
            </div>
        ...
        @forelse (getTracks($release->track_ids) as $index=>$track)
            <div class="card mt-4">
                <div class="card-body p-0">
                    <div class="py-4 px-lg-4 px-2 border-bottom">
                        <p class="tab-title mt-0 mb-2">{{++$index}}. {{$track->title}} </p>
                        <div class="release-data">
