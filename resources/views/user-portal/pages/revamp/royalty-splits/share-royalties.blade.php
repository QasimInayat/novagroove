...
@push('title') Share Royalties - @endpush
@push('styles')
    <style>
                ...
                <div class="container p-4">
                    <div class="row">
                        <div class="col-md-12 text-center">
                            ...
                            <p>Add & Edit royalty splits to automatically share the money you earn<br>from this release with your collaborators.</p>
                        </div>
                        <hr>
                        <div class="col-md-12 text-right">
                            <h4 style="color:var(--primary);">Release</h4>
                        </div>
                        <div class="col-12 border border-primary p-3 mt-1">
                            <div class="row">

                                <div class="col-md-2">
                                    <img src="{{asset(config('upload_path.album_artwork').$release->album_artwork)}}" width="100" alt="" class="d-block img-fluid">
                                </div>
                                <div class="col-md-5">
                                    <h5 style="color:var(--primary);">{{ $release->user->name }}</h5>
                                    <span>Account Owner<br>
                                    {{ $release->user->email }} (me)
                                    </span>
                                </div>
                                <div class="col-md-2 ">
                                    <span style="color:var(--primary);">
                                        {{ count(getTracks($release->track_ids)) }}
                                    </span>
                                    Tracks
                                </div>
                                <div class="col-md-3 ">
                                    Release Date <strong style="color:var(--primary);">{{getDateFormat($release->release_date)}}</strong><br>
                                    UPC CODE <strong style="color:var(--primary);">{{isset($release->upc_code) ? $release->upc_code : ''}}</strong><br>
                                    LABEL <strong style="color:var(--primary);">{{isset($release->label) ? $release->label : ''}}</strong><br>
                                    Version <strong style="color:var(--primary);">{{isset($release->version) ? $release->version : 'Original' }}</strong>
                                </div>
                        ...
                        @forelse (getTracks($release->track_ids) as $index=>$track)
                            ...
                            <div class="col-12 border border-primary p-3 mb-2" id="split_{{ +$index + 1 }}">
                                <div class="row" >
                                    ...
                                    <div class="col-md-5">
                                        <h5 style="color:var(--primary);">{{$track->title}}</h5>
                                    </div>
                                    <div class="col-md-2 ">
                                        My track split<br>
                                        <div class="progress">
                                    ...
                                    <div class="col-md-3 text-center">
                                        <button type="button" onClick="adddSplit($(this),{{ $track->id }})" data-count="{{ $index }}"  class="btn btn-primary"><i class="fa fa-plus" ></i>&nbsp;@if($trackSplit < 100 )Edit Split @else Add Split @endif</button>
                                    </div>
...
<script>
    ...
    function splitPercentage(elm){
        var count = elm.attr('data-count');
        var trackSplit = parseInt($("#trackProgress").val());
        ...
        if(actual < 0){
            ...
            var actualPercentage =  trackSplit - percentage;
            if(percentage > 100){
                notify("Percentage can not be greater than "+trackSplit,"error");
